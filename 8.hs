...
check::Map.Map k v -> String -> Bool
check regs line
    | operand == ">" = (Map.lookup reg regs) > value
    | operand == ">=" = Map.lookup reg regs >= value
    | operand == "<" = Map.lookup reg regs < value
    | operand == "<=" = Map.lookup reg regs <= value
    | operand == "==" = Map.lookup reg regs == value
--    | operand == "!=" = Map.lookup reg regs <> value
    where operand:reg:value = splitOn " " line
           

-- interpretLine::Map.Map -> String -> String -> Int
-- interpretLine regs line
--     | check regs condition = 
--     where (instruction, condition) = splitOn " if " line


main = print $ splitOn " " "ls >= -540"