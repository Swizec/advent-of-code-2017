...
def check(regs, condition):
    reg, operand, value = condition.split(" ")

    if reg not in regs:
        regs[reg] = 0

    print reg, operand, int(value)
    value = int(value)
...
def interpret(regs, operation):
    reg, operand, value = operation.split(" ")
...
input = [line.split(" if ") for line in input]
