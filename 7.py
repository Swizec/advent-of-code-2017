...
# STAR 1
# Looking for node that isn't supported by any other node

nodes = set(row.split(' (')[0] for row in input.split("\n"))

for row in input.split("\n"):
    if ' -> ' in row:
        node, supporting = row.split(' -> ')
        supporting = set(supporting.split(', '))
        nodes = nodes - supporting

print nodes


# STAR 2
# Now actually have to do tree stuff

#that's a problem for some other day